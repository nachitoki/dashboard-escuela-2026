...
@media (max-width: 700px) {
    ...
    .schedule-mobile-view { display: flex; flex-direction: column; gap: 1rem; }
    
    .mobile-day { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
    .mobile-day-title { background: var(--surface2); padding: 0.75rem 1rem; font-weight: 700; text-transform: uppercase; font-size: 0.85rem; border-bottom: 1px solid var(--border); color: var(--gold); }
    .mobile-block { display: flex; align-items: stretch; border-bottom: 1px solid var(--border); }
    .mobile-block:last-child { border-bottom: none; }
    .mobile-time { width: 80px; padding: 0.75rem; font-size: 0.7rem; color: var(--text-muted); text-align: center; border-right: 1px solid var(--border); display: flex; flex-direction: column; justify-content: center; background: rgba(0,0,0,0.2); }
    .mobile-content { flex: 1; padding: 0.75rem; }
    .mobile-content .schedule-cell { min-height: 70px; height: 100%; justify-content: center; }
}
...
.schedule-desktop-view { display: block; overflow-x: auto; padding-bottom: 1rem; }
...
function renderSchedule() {
    ...
    // VISTA MÓVIL (CARTAS POR DÍA)
    html += `<div class="schedule-mobile-view">`;
    dias.forEach(d => {
        let hasClasses = false;
        bloques.forEach(b => { if (HORARIO.semana[d][b]) hasClasses = true; });
        if (!hasClasses) return;
        
        html += `<div class="mobile-day">`;
        html += `<div class="mobile-day-title">${d}</div>`;
        bloques.forEach(b => {
            const data = HORARIO.semana[d][b];
            if (data) {
                let css = 'schedule-cell';
